...
import { industryImages, sectionContainer, sectionSpacing } from "@/lib/landing-content";

type IndustrySectionProps = {
  content: Dictionary["industries"];
};

export default function IndustrySection({ content }: IndustrySectionProps) {
            ...
            return (
              ...
              >
                ...
                <div className="absolute inset-x-0 bottom-0 p-5 text-white md:p-6">
                  ...
                  <p className={`mt-3 max-w-xl text-sm text-white/88 transition-all duration-[950ms] ease-[cubic-bezier(0.16,1,0.3,1)] ${active ? "translate-y-0 opacity-100 delay-150" : "pointer-events-none translate-y-2 opacity-0 delay-0"}`}>{description}</p>
                </div>
              </div>
            );
          })}
